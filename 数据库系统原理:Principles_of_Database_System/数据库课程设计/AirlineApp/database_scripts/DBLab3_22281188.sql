...
-- ) COMMENT='乘客信息表';
-- Snippet from your DBLab3_22281188.sql, modified for AUTO_INCREMENT
DROP TABLE IF EXISTS Passenger; -- Ensure this is safe for your current DB state
CREATE TABLE Passenger (
    passenger_id INT AUTO_INCREMENT PRIMARY KEY NOT NULL COMMENT '乘客ID',
    name VARCHAR(100) NOT NULL COMMENT '姓名',
    id_card_number VARCHAR(18) NOT NULL UNIQUE COMMENT '身份证号',
    phone_number VARCHAR(20) NOT NULL COMMENT '联系电话',
    email VARCHAR(100) UNIQUE COMMENT '电子邮件',
...
-- Add necessary indexes if not already present (as in your original file)
ALTER TABLE Passenger ADD INDEX idx_passenger_phone (phone_number);
-- Add other indexes on id_card_number, email, frequent_flyer_number as they are UNIQUE
ALTER TABLE Passenger ADD INDEX idx_passenger_id_card (id_card_number);
ALTER TABLE Passenger ADD INDEX idx_passenger_email (email);
ALTER TABLE Passenger ADD INDEX idx_passenger_ffn (frequent_flyer_number);
...
CREATE TABLE Route (
    ...
    FOREIGN KEY (airline_id) REFERENCES Airline(airline_id)
        ON DELETE RESTRICT ON UPDATE CASCADE -- Example FK constraints behavior
) COMMENT='航线信息表';
...
CREATE TABLE Booking (
    ...
    FOREIGN KEY (passenger_id) REFERENCES Passenger(passenger_id)
        ON DELETE CASCADE ON UPDATE CASCADE, -- If passenger deleted, maybe delete booking? Adjust as needed.
    FOREIGN KEY (flight_id) REFERENCES Flight(flight_id)
        ON DELETE RESTRICT ON UPDATE CASCADE -- Can't delete flight if bookings exist
) COMMENT='预订信息表';
...
CREATE TABLE Payment (
    ...
    FOREIGN KEY (booking_id) REFERENCES Booking(booking_id)
        ON DELETE RESTRICT ON UPDATE CASCADE -- Can't delete booking if payment exists (usually)
) COMMENT='支付信息表';
...
CREATE TABLE Ticket (
    ...
    FOREIGN KEY (flight_id) REFERENCES Flight(flight_id)
        ON DELETE RESTRICT ON UPDATE CASCADE,
    FOREIGN KEY (passenger_id) REFERENCES Passenger(passenger_id)
        ON DELETE RESTRICT ON UPDATE CASCADE -- Don't delete passenger if they have tickets? Or set NULL? Adjust as needed.
) COMMENT='机票信息表';
...
CREATE TABLE FlightStatus (
    ...
    FOREIGN KEY (flight_id) REFERENCES Flight(flight_id)
        ON DELETE CASCADE ON UPDATE CASCADE -- If flight is deleted, its status is removed.
