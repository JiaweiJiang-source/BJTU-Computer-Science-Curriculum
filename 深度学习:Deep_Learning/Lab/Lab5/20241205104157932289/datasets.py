...
class TrafficDataset:
    ...
    def construct_set(self, train_por=0.6, test_por=0.2, window_size=12, target_col=0, batch_size=32):
        train_x, train_y = [], []
        val_x, val_y = [], []
        test_x, test_y = [], []
        ...
        for i in range(len_train - window_size):
            train_x.append(train_data[i:i + window_size])  # 序列数据
            train_y.append(train_data[i + window_size, target_col])  # 预测值

        # 构造验证集
        val_data = self.data[len_train:len_val]
        for i in range(len_val - len_train - window_size):
            val_x.append(val_data[i:i + window_size])
            val_y.append(val_data[i + window_size, target_col])

        # 构造测试集
        test_data = self.data[len_val:]
        for i in range(len(test_data) - window_size):
            test_x.append(test_data[i:i + window_size])
            test_y.append(test_data[i + window_size, target_col])
